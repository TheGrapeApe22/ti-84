...
static char number_character(uint8_t key) {
    switch (key) {
        case sk_0: return '0'; case sk_1: return '1'; case sk_2: return '2';
        case sk_3: return '3'; case sk_4: return '4'; case sk_5: return '5';
        case sk_6: return '6'; case sk_7: return '7'; case sk_8: return '8';
        case sk_9: return '9'; case sk_DecPnt: return '.'; case sk_Add: return '+';
        case sk_Sub: return '-'; case sk_Mul: return '*'; case sk_Div: return '/';
        case sk_Power: return '^'; case sk_LParen: return '('; case sk_RParen: return ')';
        default: return '\0';
    }
}

static void reset_input(void) { input[0] = '\0'; input_length = 0; cursor_position = 0; }
static void insert_character(char c) {
    if (c && input_length < INPUT_CAPACITY - 1) {
        ...
        input[cursor_position++] = c; input_length++;
    }
}
...
static void recall_history(void) {
    ...
    text = prompt == PROMPT_HAVE ? history[(uint8_t)selected_history].have : history[(uint8_t)selected_history].want;
    while (*text && input_length < INPUT_CAPACITY - 1) insert_character(*text++);
    selected_history = -1;
}
...
static void handle_key(uint8_t key) {
    ...
    if (key == sk_Left) { selected_history = -1; if (cursor_position) cursor_position--; return; }
    if (key == sk_Right) { selected_history = -1; if (cursor_position < input_length) cursor_position++; return; }
    if (key == sk_Up) {
        if (!history_count) return;
        if (selected_history < 0) selected_history = (int8_t)history_count - 1;
        else if (selected_history > 0) selected_history--;
        return;
    }
    if (key == sk_Down) {
        if (selected_history < 0) return;
        if (selected_history < (int8_t)history_count - 1) selected_history++;
        else selected_history = -1;
        return;
    }
...
int main(void) {
    ...
    gfx_Begin(); gfx_SetDrawBuffer(); gfx_SetTextScale(1, 2);
    gfx_SetTextBGColor(COLOR_BACKGROUND); gfx_SetTextTransparentColor(COLOR_BACKGROUND);
    ...
    draw_screen(); gfx_SwapDraw();
    for (;;) {
        if (boot_CheckOnPressed() && uppercase_once) break;
        key = os_GetCSC();
        if (key) { handle_key(key); draw_screen(); gfx_SwapDraw(); }
    }
    gfx_End(); return 0;
}
