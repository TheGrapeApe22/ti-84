...
static void submit_input(void)
{
	...
	if (!input_length)
	    return;
	if (!units_validate_have
	    (input, prompt_error, sizeof(prompt_error)))
	    return;
	prompt_error[0] = '\0';
...
static void history_layout(uint8_t index, char *line,
			   uint8_t *command_lines, uint8_t *result_lines)
{
    if (history[index].want[0])
	snprintf(line, INPUT_CAPACITY * 2 + 5, "%s -> %s",
		 history[index].have, history[index].want);
    else
...
static void draw_screen(void)
{
	...
	last =
	    selected_history >=
	    0 ? (uint8_t) selected_history : history_count - 1;
	first = last;
	...
	for (index = first; index <= last; index++) {
	    ...
	    y = draw_wrapped(history[index].result, 20, y, 296,
			     result_lines, COLOR_MUTED) + 4;
	}
    if (!history_count) {
	if (database_ready)
	...
	snprintf(have_line, sizeof(have_line), "You have: %s",
		 pending_have);
