...
static bool lookup(const char *name, measure_t *out,
		   const unit_t **matched, const prefix_t **used_prefix)
{
...
static bool parse(const char *text, measure_t *out, char *error,
		  size_t cap)
{
    parser_t p = { text, "" };
    if (!parse_expression(&p, out)) {
...
static bool add_line(char *line, char *error, size_t cap)
{
    ...
    *line++ = '\0';
	...
	|| strlen(definition) >= DEFINITION_CAPACITY) {
	...
	if (existing) {
	    copy_text(existing->definition, DEFINITION_CAPACITY,
		      definition);
	    existing->state = 0;
	    return true;
	}
    ...
    copy_text(units[unit_count].definition, DEFINITION_CAPACITY,
	      definition);
...
bool units_describe(const char *have, char *result, size_t cap)
{
	...
	if (prefix) {
	    ...
	    snprintf(result, cap, "%s = %s = %s", unit->name,
		     unit->definition, normalized);
